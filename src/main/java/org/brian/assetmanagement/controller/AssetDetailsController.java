...
public class AssetDetailsController extends AbstractTemplateController {
	...
	private ObservableList<String> assetTypes = FXCollections.observableArrayList("Laptop", "PC", "Mouse", "Keyboard",
			"Monitor", "Printer", "Projector", "Docking Station", "Router", "Cable", "Connector");
	...
	private boolean validateAssetValues() {
		...
		return validate("id", id.getText(), "^[\\d\\s]+$")
				// no need to validate values from type combo box as it is populated from the
				// constants specified during assetTypes ObservableList creation
